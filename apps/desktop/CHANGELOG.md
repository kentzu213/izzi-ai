...
- Build platform assets into a draft release, then publish only after the full
  inventory passes instead of using the builder's concurrent publisher.
- Give Windows and macOS artifacts one stable `Izzi-AI-*` basename that matches
  the updater metadata generated by Electron Builder.
- Provision pnpm 9 macOS x64/arm64 optional dependencies through the temporary
  root manifest so both Sharp native runtimes reach the packaged app.
- Fail the release workflow unless all 12 expected assets have non-empty sizes
  and GitHub SHA-256 digests.
