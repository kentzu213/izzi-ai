...
function senderEvent(options: { known?: boolean; child?: boolean } = {}): IpcMainInvokeEvent {
  ...
  const url = 'http://localhost:5173/work';
...
function setup() {
  ...
  const identity: WorkIpcIdentity = {
    resolveReviewerHash: vi.fn(() => identityState.reviewerHash),
    resolveTenantWorkspaceBindings: vi.fn(() => identityState.bindings),
  };
  ...
  const call = <T>(channel: string, ...args: unknown[]): T => {
    ...
    return handler(senderEvent(), ...args) as T;
  };
...
describe('IPC sender trust', () => {
  it('accepts the trusted top-level renderer', () => {
    const { personalRun, call, close } = setup();
    const runs = call<Array<{ id: string }>>('work:listRuns', { workspaceId: 'personal' });
    expect(runs.map((run) => run.id)).toContain(personalRun.id);
    close();
  });

  it('rejects an unknown sender before parsing or resolving identity in every handler', () => {
    const { identity, close } = setup();
    ...
    for (const handler of electronMocks.handlers.values()) {
      expect(() => handler(senderEvent({ known: false }), unreadableInput)).toThrow(
        'Work IPC sender is not trusted',
      );
    }
    expect(identity.resolveReviewerHash).not.toHaveBeenCalled();
    expect(identity.resolveTenantWorkspaceBindings).not.toHaveBeenCalled();
    close();
  });

  it('rejects a child frame before parsing or resolving identity in every handler', () => {
    const { identity, close } = setup();
    const unreadableInput = new Proxy({}, {
      get: () => {
        throw new Error('input was parsed');
