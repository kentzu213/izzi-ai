...
export class CustomerMarketingService {
  ...
  private readonly referenceWorkspaceEvidenceKey = randomUUID();
  ...
  private referenceWorkspaceEvidenceDigest(material: string): string {
    // Keep the schema-v1 wire prefix for compatibility. The digest bytes are
    // HMAC-bound to this main-process service instance, so renderer code cannot
    // mint valid host evidence from public scope/package fields alone.
    return `sha256:${createHmac('sha256', this.referenceWorkspaceEvidenceKey)
  ...
  async getReferenceWorkspaceEvidence(
    ...
    const evidence: MarketingWorkspaceHostEvidence = {
      ...
      evidenceDigest: this.referenceWorkspaceEvidenceDigest(digestMaterial),
      issuedAt,
      scope,
      role: record.role,
      installedPackage,
  ...
  async provisionReferenceWorkspace(
    ...
    const currentDigest = this.referenceWorkspaceEvidenceDigest(currentMaterial);
    if (
      !this.authorityValueMatches(parsed.evidenceDigest, currentDigest)
  ...
  async saveProductMarketingContext(
    ...
    if (!this.authorityValueMatches(parsed.authorityToken, authority.authorityToken)) {
      return {
        ok: false,
        status: 'conflict',
        context: current,
