...
Scope: which text-to-speech provider Izzi AI may use for **commercial** Customer Marketing
...
Runtime path: `extensions/voice-studio/service/backend/model_runtime.py` verifies
`vieneu==3.2.3`, the audited preset catalog, and immutable model/codec revisions before
constructing `Vieneu(mode="v3turbo", backend="onnx", device="cpu")`. The backend runs on the
operator's machine, so no audio leaves the host.
...
| `pnnbao-ump/VieNeu-TTS-v3-Turbo@75ff82a72f54d55ed389e1eeb12041d3c4bac7d4` | checkpoint package | Apache-2.0 | <https://huggingface.co/pnnbao-ump/VieNeu-TTS-v3-Turbo> |
| `OpenMOSS-Team/MOSS-Audio-Tokenizer-Nano-ONNX@ceff0d0749bfb3fa2d61149794ec6feef0d1e1ae` | audio codec used by v3 Turbo | Apache-2.0 (MOSS-TTS family) | <https://github.com/OpenMOSS/MOSS-TTS> |
...
Other runtime dependencies pulled by `vieneu` 3.2.3 on the CPU path include `onnxruntime`,
`soundfile`, `soxr`, `tokenizers`, `huggingface_hub`, and `gradio`. VieNeu's wheel incorrectly
declares optional `perth>=0.2.0`; the PyPI distribution with that name is unrelated and has no
compatible wheel. The image installs a complete hash lock with `--no-deps`, removes exactly
that known metadata line, updates `RECORD`, fails closed on metadata drift, verifies Perth is
absent, and runs `pip check`. The `neucodec` codec used by v1/v2 is not installed.

**Watermarking behaviour:** inference explicitly passes `apply_watermark=False`, the Perth
module is absent, and the image labels record `io.izzi.voice.watermark=disabled`. Commercial
output from this runtime is therefore not intentionally watermarked by the optional VieNeu
integration.
...
permitted: advertising or any commercial Izzi AI/IzziAPI render or publish.
...
STARIZZI_VOICE_STUDIO_MODEL_ID=pnnbao-ump/VieNeu-TTS-v3-Turbo@75ff82a72f54d55ed389e1eeb12041d3c4bac7d4
...
it booted in the current session, so after restarting Izzi AI the container may still be running
