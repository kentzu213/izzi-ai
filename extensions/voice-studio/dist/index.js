...
function validVoiceList(data) {
  if (!data || typeof data !== 'object' || Array.isArray(data)) return null;
  var keys = Object.keys(data).sort();
  if (keys.length !== 1 || keys[0] !== 'voices' || !Array.isArray(data.voices)) return null;
  if (data.voices.length < 1 || data.voices.length > MAX_VOICE_COUNT) return null;
  var seen = Object.create(null);
  for (var i = 0; i < data.voices.length; i += 1) {
    var voice = data.voices[i];
    if (typeof voice !== 'string' || !VOICE_PATTERN.test(voice) || seen[voice]) return null;
    seen[voice] = true;
  }
  return data.voices;
}
...
module.exports = {
  ...
  commands: {
    ...
    'voice-studio.listVoices': async function () {
      ...
      var voices = validVoiceList(result.data);
      if (!voices) return { ok: false, error: 'invalid-voice-list' };
    ...
    'voice-studio.tts': async function (args) {
      ...
      if (
        !data
        || Object.keys(data).sort().join(',') !== 'audio_b64,format,ok'
        || data.ok !== true
        || data.format !== 'wav'
